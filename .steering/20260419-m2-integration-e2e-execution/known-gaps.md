...
| ID | タイトル | 影響範囲 | 対処 |
|---|---|---|---|
| GAP-1 | WorldRuntime↔Gateway 配線欠落 | Avatar live 不可 | **M4 `gateway-multi-agent-stream` で対処** |
| GAP-2 | Godot live の自動化なし | リグレッション検出不可 | M7 で検討 |
| GAP-3 | `/health` counter 監視なし | silent failure リスク | T20 acceptance に追加 |
| GAP-4 | Godot 4.6 diff の大きさ | レビュー負担 | 記録のみ |
| GAP-5 | `_NullRuntime` 説明の docs 欠落 | 初見ハマり | T20 docs 更新で対処 |
