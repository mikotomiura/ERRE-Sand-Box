...
| 5 | Godot 3-avatar 30Hz | fps 28–32 を 60s 維持 (目視) | ⏸ PENDING | MacBook 側で追 commit 予定 |
...
### #5 Godot 3-avatar 30Hz — PENDING

本項目は MacBook の Godot エディタ/ビルド必要のため本 commit 範囲外。

**実施手順 (MacBook 側)**:

1. MacBook の Godot プロジェクト (`godot_project/`) を開く
2. gateway URL を `ws://192.168.3.85:8000/ws/observe?subscribe=a_kant_001,a_nietzsche_001,a_rikyu_001` に設定
   (G-GEAR IP = 192.168.3.85, Wi-Fi interface)
3. 60s 間 3 avatar を走らせながら録画
4. fps counter が 28-32 を維持していることを目視確認
5. 録画ファイルを `.steering/20260420-m4-acceptance-live/evidence/godot-3avatar-<ts>.mp4` として配置
6. 本 acceptance.md の #5 行を ✅ PASS に更新し、追 commit

**Gateway を継続起動のまま (G-GEAR 側)** にしているので、MacBook 側のタイミングで接続可能。
必要なら `uv run erre-sandbox ...` を再起動する (前回と同コマンド)。
...
1. 本 commit (evidence 1-4 + acceptance.md): `chore(m4): live validation evidence — 3-agent acceptance (#1-#4)`
2. MacBook 側で #5 を実施、追 commit: `chore(m4): live validation evidence — godot 30hz (#5)`
3. 全項目 PASS 確定後、PR 作成 → merge → `v0.2.0-m4` タグ付与判断をユーザーへ仰ぐ
