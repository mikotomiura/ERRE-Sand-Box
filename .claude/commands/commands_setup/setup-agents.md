---
description: >
  9 + 1 (任意) のサブエージェントを .claude/agents/ 配下に構築する。
  ...
  承認を得ながら作成する。Phase 5 (Codex 連携) を実施した場合のみ、
  cross-reviewer (code-reviewer + codex-review 並列オーケストレータ) を追加する。
  各エージェントは Phase 4 で作成された Skill を参照する設計。
  /setup-skills の完了後 (Codex 連携を含むなら /setup-codex-bridge の完了後) に実行する。
allowed-tools: Read, Write, Glob, Bash(mkdir *), Bash(ls *), Bash(cat *)
---

# /setup-agents — サブエージェント群構築コマンド

> Phase 6 of 9. Let's think step by step.
...
Phase 5 (`/setup-codex-bridge`) の状態も確認:

- **完了している**: cross-reviewer エージェントを追加で作成する (本コマンド最終 Step)
- **`[~] skipped`**: cross-reviewer は作成しない (codex-review Skill が無いため動かない)
- **未完了**: ユーザーに「Phase 5 を実行するか、skip を確定するか」を確認してから進む

### Check 2: Skill の存在

```bash
ls .claude/skills/
ls .claude/skills/codex-review/SKILL.md 2>/dev/null && echo "[ok] codex-review available"
ls .claude/skills/codex-rescue/SKILL.md 2>/dev/null && echo "[ok] codex-rescue available"
```

Phase 4 で作成された Skill が存在することを確認。サブエージェントはこれらを参照する。Phase 5 完了時は `codex-review` Skill も列挙されているはず。
...
### Step 3: 9 + 1 (任意) のサブエージェントを 1 つずつ作成

> **重要**: 必ず 1 つずつ作成し、各エージェント完成後に **(a) ユーザー承認** + **(b) Grill me ステップ** を実施。
> 順序は依存の少ないものから（情報収集系 → レビュー系 → 実行系 → cross-reviewer (任意)）。
> cross-reviewer (3.10) は Phase 5 完了時のみ作成。Phase 5 を skip した場合は 3.1-3.9 の 9 個のみ。
...
#### 3.10 cross-reviewer (任意・Phase 5 完了時のみ)

> **Skip 条件**: Phase 5 (`/setup-codex-bridge`) を skip した場合、または `.claude/skills/codex-review/SKILL.md` が存在しない場合は、このエージェントを作成しない。Step 4 (整合性レビュー) に進む。

`code-reviewer` (Claude) と `codex-review` Skill (Codex) を並列で動かし、両者のレビュー結果を比較して統合レポートを返すオーケストレータ。

```markdown
---
name: cross-reviewer
description: >
  Claude (code-reviewer) と Codex (codex-review Skill) で同じ git diff を独立にレビューさせ、
  両者の結果を比較した統合レポートを返す。/cross-review コマンドから呼ばれる、または
  ユーザーが「両方の視点でレビューして」と明示した時に起動する。
  両者の指摘が一致した問題は優先度高、片方のみ指摘した問題は false positive 判定が必要。
  予算ガード (codex-budget-guard hook、.codex/budget.json) と機密フィルタ
  (secrets-pre-filter hook、scripts/secrets-filter.sh) は呼び出し前に効いている前提。
tools: Read, Bash(scripts/run-codex-review.sh *), Bash(jq *), Bash(git diff *), Bash(git diff --name-only *), Task
model: sonnet
---
...
同じ git diff を **Claude と Codex に独立にレビューさせて結果を比較する**。
Claude の自己バイアスを Codex の独立視点で補正する。
両者を直接呼ぶのではなく、code-reviewer エージェントと codex-review Skill 経由で動かす。

## 起動条件

- `/cross-review` コマンドから呼ばれた
- ユーザーが「両方の視点でレビューして」「Codex にもセカンドオピニオン」と明示した
- 重要 PR (リリース直前、セキュリティ関連、proprietary 列除く) で第二意見が欲しい時

## 適用しないケース

- diff が proprietary plugin 出力 / 機密情報を含む (`docs/external-skills.md` の「外部 LLM 送信禁止」を参照)
- `.codex/budget.json` の予算余裕が `per_invocation_max` 未満
- diff が `manual_approval_threshold_lines` (デフォルト 2000 行) を超え、ユーザー承認なし
- Phase 5 が skip されていて `codex-review` Skill が存在しない (この場合 code-reviewer 単独に切り替え)
...
# proprietary / 機密混入チェック
if git diff --name-only | grep -E '\.(pdf|docx|xlsx|pptx|env|pem|key)$|credentials'; then
  echo "[cross-reviewer] BLOCKED: diff contains proprietary or sensitive files"
  exit 1
fi

# 予算チェック (codex-budget-guard hook も同じことをするが二重防衛)
USED=$(jq -r --arg d "$(date +%Y-%m-%d)" 'if .today.date == $d then .today.tokens_used else 0 end' .codex/budget.json)
...
if [ $((BUDGET - USED)) -lt "$PER_INV" ]; then
  echo "[cross-reviewer] BLOCKED: budget exceeded"
  exit 1
fi
\`\`\`

### Step 2: 並列風レビューの起動 (疑似並列)

> **「並列」の正確な意味**: Claude Code の Task tool dispatch (Claude エージェント呼び出し) と Bash tool 実行は、メインエージェントが同一ターン内で両方の tool call を含むメッセージを発行することで「**疑似並列**」になる (Anthropic API 側のレスポンスは順次受信)。完全な非同期並列ではない。コスト削減のため、片方の結果が出てから他方を起動する **逐次** ではなく、メッセージ 1 通で両方 dispatch することは可能。

実装としては cross-reviewer が以下 2 つの tool call を **同一メッセージ** で発行する:

1. `Task(code-reviewer)` で Claude のレビューを起動
2. `Bash(scripts/run-codex-review.sh)` で Codex の wrapper を起動

`scripts/run-codex-review.sh` は本コマンド (Phase 6 / setup-agents) ではなく Phase 5 (`/setup-codex-bridge`) の生成物で、内部で:

\`\`\`bash
git diff | scripts/secrets-filter.sh > /tmp/cross-review-input.diff || exit 2
codex exec ... < /tmp/cross-review-input.diff > /tmp/cross-review-codex.md
trap 'rm -f /tmp/cross-review-input.diff' EXIT
\`\`\`

を実行する。**cross-reviewer に直接 `Bash(codex *)` を許可しない** ことで、任意コマンド実行面を絞る。

両者の結果ファイル (`/tmp/cross-review-claude.md` と `/tmp/cross-review-codex.md`) を読んで Step 3 の比較に進む。

### Step 3: 結果の比較

両者のレビュー結果を以下の 3 カテゴリに分類:

1. **両者一致 (high priority)**: 両方が指摘した問題。バイアスの可能性が低い。即対応推奨
2. **Claude のみ (要 Codex 視点での再評価)**: Codex が見落としたか、Claude の false positive か
3. **Codex のみ (要 Claude 視点での再評価)**: Claude が見落としたか、Codex の false positive か

各カテゴリに分類した上で、優先度判定 (HIGH / MEDIUM / LOW) を付ける。
...
### 両者一致 (HIGH priority)
- [指摘 1]: Claude/Codex 両方が指摘。即対応推奨。
- ...

### Claude のみ指摘 ([HIGH/MEDIUM/LOW])
- [指摘 1]: Claude の指摘。Codex は触れていない。
  - 判定: [採用 / 却下 / 要確認]
  - 理由: ...

### Codex のみ指摘 ([HIGH/MEDIUM/LOW])
- [指摘 1]: Codex の指摘。Claude は触れていない。
  - 判定: [採用 / 却下 / 要確認]
  - 理由: ...

### 両者の要約所感
- Claude: [Claude のレビュー全体所感を 1-2 行]
- Codex: [Codex のレビュー全体所感を 1-2 行]
...
`.codex/budget.json` の `tokens_used` 更新は `codex-review` Skill 内部で実施済 (atomic + flock)。本エージェントは追加更新しない。

## レポート形式

統合レポートは **150 行以内**。それを超える場合は両者のレポートを別ファイル (`/tmp/cross-review-claude.md` / `/tmp/cross-review-codex.md`) として残し、本レポートにはサマリのみ記載する。

## 参照すべき Skill

- `codex-review` (.claude/skills/codex-review/SKILL.md)
- `code-reviewer` の参照 Skill 群 (test-standards / [language]-standards / error-handling 等)

## 注意

- 両者の指摘が **大幅に乖離している** 場合は、レビュー前提 (PR の意図、関連 docs の有無) を疑う。Claude と Codex 双方に context が足りていない可能性
- Codex が `[REDACTED]` を多く含む diff を返してきた場合、secrets-filter が過剰反応している可能性。手動で抜粋して codex-review を再実行
```
...
`.steering/_setup-progress.md` の Phase 6 を完了マーク + 相互参照マップを更新:

```markdown
- [x] **Phase 6: /setup-agents** — サブエージェント群
  ...
  - 作成エージェント (9 + 1 任意):
    ...
    - log-analyzer (sonnet)
    - cross-reviewer (sonnet) ← Phase 5 完了時のみ。skipped の場合は記載しない
...
- codex-review → cross-reviewer (Phase 5 完了時のみ)
```
...
作成したサブエージェント: 9 + N 個 (N = Phase 5 完了時 1、skip 時 0)
...
- オーケストレータ: N 個 (cross-reviewer、Phase 5 完了時のみ)
...
- [ ] 9 つの基本サブエージェントすべてが作成されている
- [ ] Phase 5 完了時のみ cross-reviewer が追加で作成されている
- [ ] Phase 5 skip 時は cross-reviewer を作っていない (codex-review Skill が存在しないので動かない)
