...
> Phase 8 of 9. Let's think step by step.
> Hook はエージェントループの外で決定論的に動作するスクリプト。
> 「人間が毎回チェックすべきこと」を Hook に任せる。

## 設計原則: 3 層構成 (Preflight / Guard / Report) + Codex ガード層 (任意)
...
### 第 4 層 (任意): Codex ガード層 (Phase 5 完了時のみ)

`codex exec` 系の Bash 呼び出しを対象に、以下 3 つの hook で **二重防衛** を構成する:

- **`codex-budget-guard`** (PreToolUse on Bash): `codex exec` の実行前に `.codex/budget.json` を読み、当日予算余裕が `per_invocation_max` 未満ならブロック (exit 2)
- **`secrets-pre-filter`** (PreToolUse on Bash): `codex exec` の引数や stdin に `.env` / `*.pem` / `*.key` / `*credentials*` / proprietary 拡張子が含まれていればブロック
- **`token-report-stop`** (Stop): セッション終了時に `.codex/budget.json` の今日の使用量を `.steering/_token_log.md` に追記し、Claude / Codex 別のサマリレポートを表示

これらは Phase 5 (`/setup-codex-bridge`) 完了時のみ生成する。skip した場合は本層全体を作らない。Codex 関連 Skill 内部にも同等のチェックがあるが、hook で **エージェントループ外** から強制することで「Skill のロジックバグ / プロンプト改変」による回避を防ぐ。
...
Phase 7 完了を確認。Phase 5 (`/setup-codex-bridge`) の状態 (完了 / `[~] skipped`) を確認:

- **完了**: 第 4 層 (Codex ガード) の 3 hook を作成する
- **skipped**: 第 4 層は作成しない (Codex を使わないので予算管理 / 秘匿フィルタ不要)
...
### Step 7.5: Codex ガード層 Hook の作成 (任意・Phase 5 完了時のみ)

> **Skip 条件**: Phase 5 (`/setup-codex-bridge`) を skip した場合、または `.codex/budget.json` が存在しない場合は、このステップ全体を skip して Step 8 へ。

Codex 関連 Bash 呼び出しを **エージェントループ外** から決定論的に守る 3 つの hook を作成する。Skill 内部のチェックと二重防衛になる。

#### 7.5a. codex-budget-guard.sh の作成
...
# PreToolUse on Bash で `codex exec` を含むコマンドを検査。
# .codex/budget.json の当日予算余裕が per_invocation_max 未満ならブロック (exit 2)。
#
# 入力: stdin に Bash tool の引数 JSON
# 出力: PASS の場合 [guard] PASS: codex budget OK / BLOCKED の場合 [guard] BLOCKED: ...
...
# 高速早期 exit: codex を含まないコマンドは即座に通過 (Bash 全発火対策)
case "$COMMAND" in
  *codex*) ;;  # 進む
  *) exit 0 ;;  # 関係ないコマンド: 即 exit 0 (PASS の echo も出さない)
esac

# 文字列レベルでさらに codex exec が含まれるか確認
if ! echo "$COMMAND" | grep -qE 'codex[[:space:]]+exec'; then
  exit 0
fi

# .codex/budget.json が無ければ (Phase 5 未完了) スキップ
if [ ! -f .codex/budget.json ]; then
  echo "[guard] PASS: .codex/budget.json absent (Phase 5 not run, no guard applies)"
  exit 0
fi

# 予算チェック
TODAY=$(date +%Y-%m-%d)
DATA=$(cat .codex/budget.json)
BUDGET=$(echo "$DATA" | jq -r '.daily_token_budget')
PER_INV=$(echo "$DATA" | jq -r '.per_invocation_max')
USED=$(echo "$DATA" | jq -r --arg d "$TODAY" 'if .today.date == $d then .today.tokens_used else 0 end')

if [ $((BUDGET - USED)) -lt "$PER_INV" ]; then
  echo "[guard] BLOCKED: codex daily budget exceeded (used=$USED budget=$BUDGET per_inv=$PER_INV)"
  echo "Run /codex-bridge --reset-budget or wait until tomorrow." >&2
  exit 2
fi

echo "[guard] PASS: codex budget OK ($((BUDGET - USED)) tokens remaining)"
exit 0
\`\`\`

\`\`\`bash
chmod +x .claude/hooks/codex-budget-guard.sh
\`\`\`

#### 7.5b. secrets-pre-filter.sh の作成
...
# PreToolUse on Bash で `codex exec` 呼び出しに機密ファイル参照 / proprietary 拡張子が
# 引数 / heredoc / リダイレクトに含まれていないかを検査。混入していればブロック (exit 2)。
...
# 機密 / proprietary パターン (denylist。fail-closed のため網羅性は scripts/secrets-filter.sh と一致させる)
DENY_PATTERN='\.env([[:space:]"]|$)|\.pem([[:space:]"]|$)|\.key([[:space:]"]|$)|credentials|/secrets/|\.pdf([[:space:]"]|$)|\.docx([[:space:]"]|$)|\.xlsx([[:space:]"]|$)|\.pptx([[:space:]"]|$)'

if echo "$COMMAND" | grep -qE "$DENY_PATTERN"; then
  echo "[guard] BLOCKED: codex exec command references secrets or proprietary file" >&2
  echo "Refusing to send sensitive content to external LLM." >&2
  echo "Filtered command (truncated):" >&2
  echo "$COMMAND" | head -c 300 >&2
  exit 2
fi

# stdin (heredoc) の中身までは hook では覗けない (tool_input から見えない場合あり)
# Skill 側で git diff | scripts/secrets-filter.sh を通している前提。
echo "[guard] PASS: codex exec command surface clean"
exit 0
\`\`\`

\`\`\`bash
chmod +x .claude/hooks/secrets-pre-filter.sh
\`\`\`

> **限界の明示**: この hook は Bash tool の `command` フィールドのみを検査する。`heredoc` で stdin に渡す内容や、シェル変数経由で間接渡しされた内容までは検査できない。Skill 側の `scripts/secrets-filter.sh` と二重で構えることで穴を埋める。

#### 7.5c. token-report-stop.sh の作成
...
# Stop hook で .codex/budget.json の今日の使用量を読み、
# .steering/_token_log.md に追記。簡単なサマリを表示。
...
TODAY=$(date +%Y-%m-%d)
DATA=$(cat .codex/budget.json)
DATE_REC=$(echo "$DATA" | jq -r '.today.date')
USED=$(echo "$DATA" | jq -r '.today.tokens_used')
INVS=$(echo "$DATA" | jq -r '.today.invocations')

# 当日でなければ何もしない (history へ移動済みのはず)
if [ "$DATE_REC" != "$TODAY" ]; then
  exit 0
fi

# 0 token なら無報告 (使ってない日はノイズにしない)
if [ "$USED" -eq 0 ]; then
  exit 0
fi

# 冪等性キー: 同一セッションで Stop が複数回走っても重複記録しないため、
# (date, used, invocations) の組をキーにする。値が前回と同じなら no-op。
KEY="$TODAY:tokens=$USED:invs=$INVS"
LAST_KEY_FILE=.steering/_token_log.last_key
if [ -f "$LAST_KEY_FILE" ] && [ "$(cat "$LAST_KEY_FILE")" = "$KEY" ]; then
  exit 0  # 既に記録済み
fi

# .steering/_token_log.md に追記 (atomic update + flock)
mkdir -p .steering
LOG=.steering/_token_log.md
LINE="- $TODAY (session end): codex tokens=$USED, invocations=$INVS"

(
  flock -x 9
  if [ ! -f "$LOG" ]; then
    {
      echo "# Codex Token Usage Log"
      echo
      echo "Generated by .claude/hooks/token-report-stop.sh on session end."
      echo "Reflects per-day codex tokens (Claude tokens are tracked separately by /context)."
      echo
    } > "$LOG"
  fi
  # 同日の重複行があれば上書き、無ければ追記
  if grep -q "^- $TODAY" "$LOG" 2>/dev/null; then
    sed -i.bak "s/^- $TODAY.*/$LINE/" "$LOG"
    rm -f "${LOG}.bak"
  else
    echo "$LINE" >> "$LOG"
  fi
...
echo "[stop] codex usage today: $USED tokens / $INVS invocations (logged to $LOG)"
exit 0
\`\`\`

\`\`\`bash
chmod +x .claude/hooks/token-report-stop.sh
\`\`\`

#### 7.5d. ユーザー確認

> 「Codex ガード層 hook を有効にしますか? PreToolUse で `codex exec` 呼び出し前に予算 / 秘匿チェックが走り、Stop hook で当日の codex token 使用量が `.steering/_token_log.md` に記録されます。」

`No` の場合は Step 9 で settings.json の Codex 関連 hook 登録だけスキップ (スクリプトファイル自体は残しておくと後から有効化しやすい)。
...
```json
{
  "hooks": {
    ...
    "PreToolUse": [
      ...
      {
        "//": "Codex ガード層 — Phase 5 完了時のみ有効。Phase 5 skip 時はこのブロックを削除",
        "matcher": "Bash",
        "hooks": [
          { "type": "command", "command": "bash .claude/hooks/codex-budget-guard.sh" },
          { "type": "command", "command": "bash .claude/hooks/secrets-pre-filter.sh" }
        ]
      }
    ],
    "Stop": [
      {
        "hooks": [
          ...
          { "//": "Codex Token レポート — Phase 5 完了時のみ有効", "type": "command", "command": "bash .claude/hooks/token-report-stop.sh" }
        ]
      }
    ],
    "PostToolUse": [
...
⚠️ **Codex 関連 hook の登録条件 (機械的に判定)**:

```bash
# Phase 5 完了時 (.codex/config.toml が存在) は登録、そうでなければ削除
...
`CODEX_HOOKS_ENABLED=false` の場合 (Phase 5 skip 状態) は **本コマンドが settings.json テンプレートから Codex 関連ブロックを物理的に削除して書き出す**。「削除推奨」のような曖昧な指示にせず、生成時点でブロックを出さないか、自動削除する:

```bash
if [ "$CODEX_HOOKS_ENABLED" = "false" ]; then
  jq 'del(.hooks.PreToolUse[] | select(.matcher == "Bash" and (.hooks[].command | tostring | test("codex"))))
      | del(.hooks.Stop[].hooks[] | select(.command | tostring | test("token-report-stop")))' \
    .claude/settings.json > .claude/settings.json.tmp \
    && mv .claude/settings.json.tmp .claude/settings.json
fi
```

> **設計原則 — 3 層責務分離**: 同じ予算 / 機密チェックが Phase 5 の Skill / Phase 7 のコマンド / Phase 8 の hook に出てくるのは意図的。役割分担:
>
> | 層 | 場所 | 役割 | 失敗時の挙動 |
> |---|---|---|---|
> | command (`/cross-review`) | Phase 7 | UX / 早期警告 | ユーザーに状態提示、フォールバック提案 |
> | hook (`codex-budget-guard` 等) | Phase 8 | 強制遮断 | exit 2 でツール呼び出しブロック |
> | wrapper / Skill (`run-codex-review.sh` / `codex-review`) | Phase 5 | 最終防衛 | 内部で exit 1/2、tmp ファイル削除 |
>
> 判定ロジックの定数 (`per_invocation_max` 等) は `.codex/budget.json` の単一の真実源を全層が読む。コードの重複ではなく **失敗モード別の独立した防衛線**。

`CODEX_HOOKS_ENABLED=true` の場合のみ、上記 settings.json の Codex 関連ブロックを残す。
...
```markdown
- [x] **Phase 8: /setup-hooks** — Hook 群（3 層 + 第 4 層 Codex ガード）
  ...
  - 作成 Hook:
    ...
    - **PreToolUse: 第 4 層 / codex-budget-guard.sh + secrets-pre-filter.sh** (Phase 5 完了時のみ、Bash matcher、`codex exec` 呼び出しを検査)
    - **Stop: 第 4 層 / token-report-stop.sh** (Phase 5 完了時のみ、当日の codex token 使用量を `.steering/_token_log.md` に記録)
  ...
  - 外部スクリプト:
    ...
    - .claude/hooks/codex-budget-guard.sh （Phase 5 完了時のみ）
    - .claude/hooks/secrets-pre-filter.sh （Phase 5 完了時のみ）
    - .claude/hooks/token-report-stop.sh （Phase 5 完了時のみ）
...
- PreToolUse Hook (Codex 層) → /cross-review および手動 codex exec 呼び出し時に予算 / 秘匿チェック (Phase 5 完了時のみ)
- Stop Hook (Codex 層) → 全コマンドのターン終了時に当日 codex token 使用量を記録 (Phase 5 完了時のみ)
```
...
設定した Hook: 5 + N 種（N = Phase 5 完了時 3、skip 時 0）
...
【Codex 層 (Phase 5 完了時のみ)】
6. PreToolUse (Bash matcher) → codex-budget-guard.sh + secrets-pre-filter.sh
   → codex exec 呼び出し前に予算 / 機密 / proprietary を検査
   → 出力: [guard] PASS: codex budget OK / [guard] BLOCKED: ...
7. Stop → token-report-stop.sh
   → 当日 codex token 使用量を .steering/_token_log.md に記録
   → 出力: [stop] codex usage today: ...
...
- [ ] Phase 5 完了時のみ: codex-budget-guard.sh / secrets-pre-filter.sh / token-report-stop.sh が実行権限付きで作成されている
- [ ] Phase 5 skip 時は: 第 4 層スクリプトを作っていない、または settings.json に登録していない
