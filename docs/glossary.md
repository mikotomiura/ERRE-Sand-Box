...
| ペルソナドリフト | Persona Drift | 対話ターン数の増加に伴い、エージェントの言動が初期設定から乖離する現象。外部 AgentState + 記憶 + LoRA の三重冗長で緩和 | 「50ターン以降のペルソナ一貫性スコアを監視する」 |
...
| DMN バイアス | DMN Bias | Default Mode Network に着想を得た発散的思考パラメータ。+1.0 = 最大発散 (idle/peripatetic)、-1.0 = 最大集中 (focused) | ERREMode, 意図的非効率性 |
...
| RAG | Retrieval-Augmented Generation | 検索拡張生成。記憶検索 + LLM 生成の組み合わせ |
...
| NPC | エージェント (Agent) | ゲーム開発向けドキュメントではNPCと対比して説明するが、本体のコード・ドキュメントでは「エージェント」を使用 |
| ボット | エージェント (Agent) | 同上 |
| チャットボット | エージェント (Agent) | 対話は認知サイクルの一部であり、チャットが主目的ではない |
| データベース (曖昧) | sqlite-vec / Memory Layer / erre.db | 具体的にどの DB を指すか明示する |
| AI モデル (曖昧) | ベースモデル / LLM / Qwen3-8B 等の具体名 | どのモデルを指すか明示する |
| サーバー (曖昧) | G-GEAR / SGLang server / FastAPI server | どのサーバーを指すか明示する |
