...
```yaml
# ✅ 良い例 — personas/kant.yaml (schemas.PersonaSpec 準拠)
...
primary_corpus_refs:
  - kuehn2001
cognitive_habits:
  - description: "15:30±5min walk 60-75min, no return conversation"
    source: kuehn2001
  ...
  - description: "nasal breathing only during walk"
    ...
    trigger_zone: peripatos
personality:                 # schemas.PersonaSpec は `personality` フィールド (not personality_traits)
  openness: 0.85
...
Note: `shuhari_stage` は静的な性格特性ではなく動的な認知状態のため、
`PersonaSpec.personality` ではなく `AgentState.cognitive` に配置される
(schemas.py §4)。ペルソナ YAML では指定しない。
